...
> 最后更新: 2026-06-10 提交前 | 清洗层: 作者感知去重+内容角色五分类+QQ噪声过滤 | 时区: 北京时间 | UI: 8页面
...
| 步骤 | 页面 | 操作 |
|------|------|------|
| 1 | **🎯 灰黑产情报分析Agent** | **一键全流程**：选平台+关键词 → 启动 → 自动采集/清洗/研判/入库（实时进度） |
| 2 | **🎣 钓鱼模拟** | 选AI人物+目标 → 流式对话 → 结构化情报提取 |
| 3 | **总览 / ChatBI** | 查看数据概览、风险分布、ChatBI 问答 |
| 4 | **研判工作台** | 查看分类+实体+证据+风险评分详情 |
| 5 | **知识库** | 浏览黑话词典、实体库、候选黑话、图谱扩线 |
| 6 | **系统状态** | 查看 MySQL/Neo4j/Milvus 连接状态 |

> **推荐开场**：先在情报分析Agent页面跑一次全流程(2分钟)，再切到钓鱼模拟跑一次对话(2分钟)，然后展示各环节独立页面。
...
| 页面 | 路由 | 功能 |
|------|------|------|
| **🎯 灰黑产情报分析Agent** | `?page=pipeline` | 一键全流程：采集→清洗→研判→入库，实时进度+ID串联 |
| **总览 / ChatBI** | `?page=overview` | 数据概览 + 风险分布 + 自然语言问答 |
| **🎣 钓鱼模拟** | `?page=persona` | 流式实时对话 + 角色自定义 + 结构化情报提取 |
| **采集器管理** | `?page=collector` | 7 平台卡片 + 一键采集 + 实时日志 |
| **数据清洗** | `?page=cleaning` | 批量清洗 + 前后预览 + 作者感知去重 + 内容角色分类 |
| **研判工作台** | `?page=workbench` | 分类/实体/证据/风险评分 |
| **情报池** | `?page=intel_pool` | 全量情报查看 + 状态筛选 + 批量研判 |
| **知识库** | `?page=knowledge` | 实体库 + 黑话词典 + 候选黑话 + 图谱扩线 |
| **系统状态** | `?page=system` | MySQL/Neo4j/Milvus/Doris 连接状态 |
