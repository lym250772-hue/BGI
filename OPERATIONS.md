...
```bash
# 内容平台
python main.py collect -p weibo -k "刷单" --max-pages 5     # AJAX API (纯HTTP)
python main.py collect -p zhihu -k "刷单,接码" --max-pages 5  # 浏览器内fetch API
python main.py collect -p tieba -k "刷单" --max-pages 2      # 浏览器+DOM
