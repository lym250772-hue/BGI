...
def search_api(page, keyword, page_num=1):
    ...
    if msToken: params.append(("msToken", msToken))
    ...
    if result.get("error"):
        logger.warning(f"  API fetch 失败: {result['error']}")
        return []

    data = result.get("data", [])
    nil = result.get("search_nil_info", {})
    logger.info(f"  API: status={result.get('status_code')} items={len(data)} nil={nil.get('search_nil_item','')}")
    ...
    with sync_playwright() as p:
        ...
        for kw in keywords:
            for pg in range(1, args.max_pages + 1):
                items = search_api(page, kw, pg)
                all_items.extend(items)
                if not items: break
                time.sleep(random.uniform(2, 4))
            logger.info(f"[{kw}] 采集 {sum(1 for i in all_items if i['keyword']==kw)} 条")
