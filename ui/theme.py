...
[data-testid="stSidebar"] .stButton > button {{
    background: transparent; color: {TEXT_SOFT}; border: 1px solid transparent;
    border-radius: 8px; padding: 0.55rem 0.8rem; font-size: 0.88rem; font-weight: 450;
    text-align: left; width: 100%; transition: all 0.15s; box-shadow: none;
    display: flex; align-items: center; gap: 0.5rem;
}}
[data-testid="stSidebar"] .stButton > button:hover {{
    background: {BG_BASE}; color: {TEXT_MAIN}; border-color: {BORDER};
}}
[data-testid="stSidebar"] .stButton > button[kind="primary"] {{
    background: {BG_CARD}; color: {TEXT_MAIN}; font-weight: 550;
    border-color: {BORDER}; box-shadow: 0 1px 3px rgba(60,58,53,0.05);
}}
...
[data-testid="stMetric"] {{
    ...
    padding: 1.1rem 1.4rem; box-shadow: 0 1px 4px rgba(60,58,53,0.03);
    transition: box-shadow 0.2s, transform 0.2s;
}}
[data-testid="stMetric"]:hover {{
    box-shadow: 0 3px 12px rgba(60,58,53,0.07);
...
[data-testid="stMetricValue"] {{
    color: {TEXT_MAIN} !important; font-size: 1.7rem !important; font-weight: 600 !important;
}}
...
[data-testid="stDataFrame"] th {{
    ...
    font-weight: 500 !important; font-size: 0.8rem !important; padding: 0.5rem 0.8rem !important;
}}
[data-testid="stDataFrame"] td {{
    font-size: 0.84rem !important; padding: 0.45rem 0.8rem !important;
...
div[data-testid="column"] .stButton > button {{
    background: {SAGE}; color: white; border: none; border-radius: 8px;
    padding: 0.5rem 1.5rem; font-weight: 500; transition: all 0.15s;
}}
div[data-testid="stVerticalBlock"] .stButton > button:hover {{
    background: {SAGE_DARK}; box-shadow: 0 2px 8px rgba(139,157,131,0.25);
}}
...
/* Selectbox dropdown */
div[data-baseweb="popover"] {{ border-color: {BORDER} !important; border-radius: 8px !important; }}
div[data-baseweb="popover"] [role="option"]:hover > div {{ background: {BG_BASE} !important; }}
div[data-baseweb="popover"] [aria-selected="true"] > div {{ background: {BG_SIDEBAR} !important; }}
...
.stCaption {{ color: {TEXT_MUTED} !important; font-size: 0.82rem !important; }}
...
[data-testid="stSpinner"] > div {{ border-top-color: {SAGE} !important; }}
div.stSpinner > div {{ border-color: {SAGE} !important; }}
...
.empty-state .title {{ font-size: 0.95rem; font-weight: 500; color: {TEXT_SOFT}; margin-bottom: 0.35rem; }}
