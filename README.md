...
数据流程：打开「灰黑产情报分析Agent」页面 → 选平台+关键词 → 一键启动 → 自动采集入库(北京时间) → 自动清洗去重(作者感知+QQ噪声过滤) → 自动研判(三级分类+实体抽取) → 多库同步。各环节拆分为独立页面仅为展示处理过程，全流程由一条流水线自动串联。
...
| Streamlit 前端 | 已实现 | 🎯情报工厂(一键全流程)、总览/ChatBI、🎣钓鱼模拟(流式对话)、采集器管理、数据清洗、研判工作台、情报池、知识库、系统状态 |
...
| 页面 | 功能 |
|---|---|
| 🎯 灰黑产情报分析Agent | **一键全流程**：选平台→选关键词→启动→自动采集(实时计数)→清洗(逐条进度)→研判(预览+耗时)→多库同步。严格ID串联只处理当次数据 |
| 总览 / ChatBI | 查看接收总量、待研判、已研判、高危情报、候选黑话；风险分布与趋势图；轻量 ChatBI 态势问答 |
| 🎣 钓鱼模拟 | **流式实时对话**：3个AI人物(电商卖家/大学生/自媒体人)可选，支持角色字段自定义编辑，对话气泡逐轮刷新，自动提取结构化情报(服务/定价/支付/风险指标) |
| 采集器管理 | 7平台独立采集器，选择关键词和页数，一键触发采集，实时展示入库结果 |
| 数据清洗 | 批量清洗 + 清洗预览 + 操作日志，展示作者感知去重、内容角色分类、噪声评分详情 |
| 研判工作台 | 选择或输入一条情报，提交异步研判任务，查看风险分类、实体、证据、黑话、扩线结果 |
| 情报池 | 查看全部情报，按状态筛选(待清洗/待研判/已研判/已丢弃)，批量提交待研判数据 |
| 知识库 | 查看实体库、黑话词典、候选黑话和关系扩线入口 |
| 系统状态 | 查看 MySQL、Neo4j、Milvus、Doris 实时连接状态和错误信息 |

### 7.1 灰黑产情报分析Agent（全自动流水线）

这是推荐比赛演示的入口页面。不同于其他页面展示单项能力，此页面整合了完整的数据处理链路：

```
选平台 + 关键词 → [Step 1: 数据采集] → [Step 2: 数据清洗] → [Step 3: 自动研判] → [Step 4: 多库写入]
```

- 采集阶段实时显示已入库条数
- 清洗阶段逐条显示通过/丢弃详情（作者感知去重 + QQ系统账号过滤）
- 研判阶段逐条显示内容和耗时（规则→RoBERTa→LLM三级级联）
- 完成后展示完整统计（采集数/清洗通过/丢弃/研判完成/高危情报）

### 7.2 钓鱼模拟（Persona Engine）

3个AI人物Profile基于 YAML 配置，每个包含完整的身份设定、对话风格、安全护栏规则和情报采集目标。对话采用流式生成器模式，每轮对话后自动刷新页面，实时展示气泡和安检结果。

6个预设场景（刷单/解封/涨粉/接码/数据/游戏黑产）+ 支持角色字段级自定义编辑（身份/语气/开场白/退出条件/采集目标），修改仅对当次对话生效。
...
├── tests/                   # 单元测试
├── persona/                 # AI人物钓鱼引擎(YAML配置+流式对话+安全护栏)
├── ui/                      # Streamlit 前端(8页面)
│   └── views/
│       ├── pipeline.py      # 🎯 全自动流水线
...
1. **开场**：打开「灰黑产情报分析Agent」页面 → 选择微博 + “刷单” → 一键启动 → 实时展示采集(18条/6s)→清洗(作者感知去重)→研判(三级分类)→多库同步的全自动流程。(2分钟)
2. **钓鱼模拟**：切到钓鱼模拟页面 → 选择”电商卖家小张” → 预设”刷单服务”场景 → 开始对话 → 流式展示对话气泡 → 提取结构化情报(服务/定价/支付方式)。(2分钟)
3. **情报池**：展示多平台情报，说明数据源已统一进入 `RAW_COLLECTED` → CLEANED → ANALYZED 状态。(1分钟)
4. **研判工作台**：打开一条高危情报，展示风险分类、实体抽取、证据片段、黑话归一、候选新黑话。(2分钟)
5. **知识库**：查看实体库和候选黑话，说明系统具备数据飞轮和人工复核入口。(1分钟)
6. **总览 / ChatBI**：问”哪个平台高危情报最多””最近热门黑话有哪些”，展示事实聚合。(2分钟)
7. **钓鱼模拟·自定义**：展示角色字段自定义编辑功能，证明系统灵活性。(1分钟)
8. **系统状态**：展示 MySQL、Neo4j、Milvus、Doris 全部在线，证明工程完整性。(1分钟)
