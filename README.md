...
- **多源采集** — 6平台 Spider+Collector（微博/知乎/贴吧/小红书/抖音/Telegram），5渠道已打通
- **智能清洗** — SimHash 去重 + 平台感知噪声过滤 + Emoji语义翻译 + OCR
- **三级分类** — L1 关键词规则 → L1.5 Metadata → L2 RoBERTa 微调 → L3 LLM 推理
- **实体抽取** — 正则 → 词典 → Milvus 向量检索 → LLM 结构化抽取四层级联
- **知识图谱** — Neo4j 构建实体关系网络，支持关联分析 & 团伙发现
- **黑话归一** — 命中词典标准释义 + 新词候选发现
- **可视化仪表盘** — Streamlit 莫兰迪配色 + 轻量 ChatBI
...
├── collectors/              # 多源数据采集层（6平台Spider+Collector）
├── config/                  # 配置与风险规则
├── data/                    # 模型、词典、样例数据
...
| 平台 | 优先级 | 状态 | 采集速度 | 技术方案 |
|------|--------|:----:|:--:|------|
| 微博 | P0 | ✅ | 12.8条/秒 | AJAX API (`requests` 直调 `weibo.com/ajax/statuses/search`) |
| 知乎 | P0 | ✅ | 0.2条/秒 | Playwright API 直调 (`page.evaluate(fetch)`) |
| 小红书 | P1 | ✅ | 0.3条/秒 | Playwright API 拦截 + DOM 兜底 |
| 抖音 | P1 | ✅ | 0.4条/秒 | 首页搜索框 + 键盘操作 + 正则文本提取 |
| 贴吧 | P1 | ✅ | 0.03条/秒 | Playwright DOM 解析 (`.thread-content-box`) |
| Telegram | P2 | ⚠️ | - | Telethon，需配置 API ID/Hash |
